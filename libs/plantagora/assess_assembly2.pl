...
my $region = 0;
my $cds = 0;
my $umt = 0.1; # threshold for misassembled contigs with aligned less than $umt * 100% (Unaligned Missassembled Threshold)
...
open (COORDS_FILT, ">$header.coords.filtered");
my $line = <COORDS>;
print COORDS_FILT "$line";
...
my $name;
my $seq;
open (ASSEMBLY, $AssFile) or die "can't open $AssFile: $!\n"; # from http://code.izzid.com/2011/10/31/How-to-read-a-fasta-file-in-perl.html
while (<ASSEMBLY>) {
	if (/>/) {
		if ($seq) {
				$counter++;
...
if ($seq) { ## be aware, it's copy-pasta from above! but it's perl, so the code should be cryptic. 
		$counter++;
		if ($counter%10000 == 0) {print "\t...$counter contigs loaded.\n";}
		if (!$name) {$name = "0"; }
		$assembly{$name} = $seq;
...
my %ref_aligns;
my %ref_features;
open (REFERENCE, $RefFile) or die "can't open $RefFile: $!\n"; # from http://code.izzid.com/2011/10/31/How-to-read-a-fasta-file-in-perl.html
while (<REFERENCE>) {
	if (/>/) {
		if ($seq) {
			$reference{$name} = $seq;
...
my %snps;
my %snp_locs;
$status = open (SNPS, "<$header.snps");
if (defined $status) { # file is opened
	while ($line = <SNPS>) {
		#print "$line";
		chomp $line;
		if ($line !~ /^\d/) {next;}
...
if ( $gff_file ){
	...
	foreach $ref (keys %reference){
		push (@{$regions{$ref}}, [1, length($reference{$ref})]);
		$total_regions++;
		$total_reg_len += length($reference{$ref});
	}
}	
...
foreach $contig (keys %assembly){
	...
	$ctg_len = length($assembly{$contig});
...
foreach $ref (keys %regions){
	...
	if (! exists $ref_aligns{$ref}) { 
		...
		print "ERROR: Alignment Reference Headers: ", @{keys %ref_aligns}, "\n";
		next; 
	}
    ...
    print "\t\tMisassembled and Unaligned: $misassembled_partially_unaligned\n";
	...
	print "\tRedundant Contigs: ", scalar(@redundant), " ($total_redundant)", "\n";
