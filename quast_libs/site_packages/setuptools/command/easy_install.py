...
from local_pkg_resources import (
...
warnings.filterwarnings("default", category=local_pkg_resources.PEP440Warning)
...
class easy_install(Command):
    ...
    __mv_warning = textwrap.dedent("""
        ...
        'import local_pkg_resources' and then use a 'require()' call similar to one of
        these examples, in order to select the desired version:

            local_pkg_resources.require("%(name)s")  # latest installed version
            local_pkg_resources.require("%(name)s==%(version)s")  # this exact version
            local_pkg_resources.require("%(name)s>=%(version)s")  # this version or higher
        """).lstrip()
...
def update_dist_caches(dist_path, fix_zipimporter_caches):
    ...
    # do so:
    # * Global setuptools local_pkg_resources.working_set (a.k.a. 'master working
...
class ScriptWriter(object):
    ...
    template = textwrap.dedent("""
        ...
        __requires__ = %(spec)r
        import sys
        from local_pkg_resources import load_entry_point
...
def load_launcher_manifest(name):
    manifest = local_pkg_resources.resource_string(__name__, 'launcher manifest.xml')
    if six.PY2:
