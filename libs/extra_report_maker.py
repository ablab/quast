...
def do(total_report, genome_info, output_filename, min_contig, json_output_dir):
    total_report += '.txt'
                ...
                if metric == '_aux Miscalled':
                    if fields[metrics_positions[id]].strip() == '':
                        ...
                        report_dict[cur_assembly].append(
                            "%.2f" % (float(fields[metrics_positions[id]]) * 100000.0 / float(genome_size)))
    ...
    #    paper_table = ''
    #    folder_with_pt = os.path.join(__location__, 'libs/report')
