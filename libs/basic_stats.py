...
def do(reference, filenames, output_dir, all_pdf):
    ...
    for id, filename in enumerate(filenames):
        ...
        lengths.append(fastaparser.get_lengths_from_fastafile(filename))
    ...
    for id, (filename, lens) in enumerate(itertools.izip(filenames, lengths)):
        n50 = N50.N50(lens)
        if reference:
            ng50 = N50.NG50(lens, reference_length)
        n75 = N50.N50(lens, 75)
        if reference:
            ng75 = N50.NG50(lens, reference_length, 75)
        ...
        report_dict[os.path.basename(filename)].append(len(lens))        
        report_dict[os.path.basename(filename)].append(max(lens))        
    ...
    import plotter
    plotter.cumulative_plot(filenames, lengths, output_dir + '/cumulative_plot', 'Cumulative length', all_pdf)
    
    ########################################################################

    # Drawing Nx and NGx plots...
    plotter.Nx_plot(filenames, lengths, output_dir + '/Nx_plot', 'Nx', [], all_pdf)
    if reference:
        plotter.Nx_plot(filenames, lengths, output_dir + '/NGx_plot', 'NGx', [reference_length for i in range(len(filenames))], all_pdf)        
