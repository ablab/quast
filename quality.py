...
def usage(release_mode=False):
    print >>sys.stderr, 'Estimation assembly quality using different metrics and tools'
    print >>sys.stderr, 'Usage:', sys.argv[0], ' [options] contig file(s)'
    print >>sys.stderr, ""

    if release_mode:
        print >>sys.stderr, "Options:"
        print >>sys.stderr, "-o\t<dirname>\tdirectory to store all result files [default: results_<datetime>]"
        print >>sys.stderr, "-R\t<filename>\tfile with reference genome"
        print >>sys.stderr, "-G/--genes\t<fname>\tfile with genes for given species"
        print >>sys.stderr, "-O/--operons\t<fname>\tfile with operons for given species"
        ...
        print >>sys.stderr, '--not-circular\t\t\t\tthis flag should be set if genome is not the circular one (e.g., eukaryote)'
        print >>sys.stderr, ""
        ...
        print >>sys.stderr, "-R\t--reference\tFile with reference genome"
        print >>sys.stderr, "-G\t--genes\tFile with genes for given species"
        print >>sys.stderr, "-O\t--operons\tFile with operons for given species"
        print >>sys.stderr, "-t\t--contig-thresholds\t_Comma-separated_ list of contig length thresholds (default is %s)" % (qconfig.contig_thresholds)
        print >>sys.stderr, "-M\t--min-contig\tLower threshold for contig length (default is %s)" % (qconfig.min_contig)
        print >>sys.stderr, "-f\t--orf\t_Comma-separated_ list of threshold lengths of ORFs to search for (default is %s)" % (qconfig.orf_lengths)
        print >>sys.stderr, "-e\t--genemark-thresholds\t_Comma-separated_ list of threshold lengths of genes to search with GeneMark (default is %s)" % (qconfig.genes_lengths)
        print >>sys.stderr, ""
        print >>sys.stderr, "Options without parameters:"
        print >>sys.stderr, '-m\t--mauve\tUse Mauve'
        print >>sys.stderr, '-g\t--gage\tUse GAGE ONLY'
        print >>sys.stderr, '-n\t--not-circular\tGenome is not circular'
...
def main(args, lib_dir=os.path.join(os.path.abspath(sys.path[0]), 'libs'), release_mode=False):
